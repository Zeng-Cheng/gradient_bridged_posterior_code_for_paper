...
B, d, n = X_data.shape
print(f"Data Loaded: B={B}, d={d}, n={n}")
...
for i in trange(num_steps):
    ...
    if i % 20000 == 0:
        print(f"Step {i}, Loss: {loss:.5f}") # loss should converge to 0

# %%
# the two matrices should match; the losses should also match
...
def model(X_data, lambda_reg=100.0):
    ...
    W_flat = numpyro.sample("W", dist.Normal(0.0, 10.0), sample_shape=(B, d * (d + 1) // 2)) # type: ignore[arg-type]
...
rtest = R_samples[0, 3, :, :]
W_b_flat_samples0 = W_flat_samples[0, 3, :]
W_b = get_W_from_flat(W_b_flat_samples0)
R_b = cal_R_from_W(W_b, X_data[3], s_samples[0, 3], u_samples[0])
reg = jnp.sum((R_b.T @ R_b - jnp.eye(d)) ** 2)
print(rtest.T @ rtest) # should be identity matrix
print(reg) # should be close to zero

svd_optimal = optimize_procrustes_svd(X_data[3], s_samples[0, 3], u_samples[0])
print(rtest)
print(svd_optimal)
print(jnp.sum((rtest @ X_data[3] - s_samples[0, 3] * u_samples[0]) ** 2))
print(jnp.sum((svd_optimal @ X_data[3] - s_samples[0, 3] * u_samples[0]) ** 2))


# %%
# Save s_samples, u_samples, and sigma2_samples
np.savetxt("res_data_application/s_samples.txt", np.array(s_samples), delimiter=",")
np.savetxt("res_data_application/u_samples.txt", np.array(u_samples).reshape(u_samples.shape[0], -1), delimiter=",")  # Flatten last two dims
np.savetxt("res_data_application/sigma2_samples.txt", np.array(sigma2_samples), delimiter=",")

# Save W_b_samples
np.savetxt("res_data_application/W_samples.txt", W_flat_samples.reshape(num_samples * B, -1), delimiter=",")

R_samples_reshaped = R_samples.reshape(num_samples * B, d * d)
np.savetxt("res_data_application/R_samples.txt", R_samples_reshaped, delimiter=",")
...
R_samples = np.loadtxt("res_data_application/R_samples.txt", delimiter=",").reshape(num_samples, B, d, d)
W_flat_samples = np.loadtxt("res_data_application/W_samples.txt", delimiter=",").reshape(num_samples, B, -1)

# %%
# Extract W_1_11 (first entry of first batch's W matrix)
W_1_11_samples = W_flat_samples[:, 0, 0]  # Extract (1,1) entry
...
print(f"DBI: {db_indices[max_ari_index]} at index: {max_ari_index}")
print(f"Maximum DB Index: {max_db_index} at index: {max_db_index_index}")


# %%
j = max_nmi_index # 265 REMEMBER to change
corrected_data = []
...
np.savetxt("res_data_application/pca_gradient-bridge.txt", all_data_pca)

all_data_umap = umap.UMAP().fit_transform(all_data)
np.savetxt("res_data_application/umap_gradient-bridge.txt", all_data_umap) # type: ignore[arg-type]
...
np.savetxt("res_data_application/db_indices_gradient-bridge.txt", db_indices_np)
...
np.savetxt("res_data_application/angles_gradient-bridge.txt", np.concatenate(angles_R))
